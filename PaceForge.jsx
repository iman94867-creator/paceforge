...
.pf-foot{ text-align:center; margin-top:50px; color:var(--muted); font-size:13px; }

.pf-lock{ margin-top:18px; background:linear-gradient(160deg,rgba(212,255,62,0.08),var(--surface)); border:1px solid var(--line); border-radius:22px; padding:34px; text-align:center; }
.pf-lock-icon{ font-size:30px; margin-bottom:10px; }
.pf-lock-list{ list-style:none; padding:0; margin:18px auto; max-width:420px; text-align:left; }
.pf-lock-list li{ padding:9px 0 9px 28px; position:relative; font-size:14.5px; border-bottom:1px solid var(--line); }
.pf-lock-list li:last-child{ border-bottom:none; }
.pf-lock-list li::before{ content:"✓"; position:absolute; left:0; color:var(--volt); font-weight:800; }
.pf-price{ font-family:'Anton'; font-size:54px; color:var(--text); margin:14px 0 4px; }
.pf-price span{ font-family:'Manrope'; font-size:15px; font-weight:700; color:var(--muted); letter-spacing:0; }
.pf-price-was{ font-family:'Anton'; font-size:30px; color:var(--muted); text-decoration:line-through; margin-right:14px; opacity:0.7; }
.pf-price-badge{ display:inline-block; background:var(--volt); color:#0a0b0a; font-weight:800; font-size:12px; letter-spacing:0.08em; text-transform:uppercase; padding:6px 14px; border-radius:100px; margin-top:6px; }
.pf-lock-cta{ margin-top:8px; }
.pf-lock-note{ font-size:12.5px; color:var(--muted); margin-top:14px; max-width:380px; margin-left:auto; margin-right:auto; line-height:1.5; }
.pf-lock-back{ display:block; margin:18px auto 0; background:none; border:none; color:var(--muted); font-family:'Manrope'; font-weight:700; font-size:14px; cursor:pointer; }
.pf-lock-back:hover{ color:var(--text); }.pf-restart{ background:none; border:1px solid var(--line); color:var(--text); padding:13px 26px; border-radius:12px; font-family:'Manrope'; font-weight:700; cursor:pointer; transition:all .15s; }
...
/* ---- CHARGING ----
   PRICE: what you show the customer.
   PAYMENT_LINK: paste your Stripe Payment Link URL here. When set, the
   "Pay & unlock" button sends the customer to Stripe; after paying, Stripe
   returns them to your site with ?paid=true and the full plan unlocks.
   Leave PAYMENT_LINK as "" while testing — the button then unlocks for free. */
const PRICE = "£9.99";
const PRICE_WAS = "£20";   // shown struck-through next to PRICE; set to "" to hide
const PAYMENT_LINK = "";

// Safe localStorage (no-ops in sandboxes that block it).
...
export default function App() {
  ...
  const [unlocked, setUnlocked] = useState(false); // has the customer paid for this plan?
  ...
  // Returning from a Stripe payment? Unlock and restore the saved plan.
  useEffect(() => {
    let params;
    try { params = new URLSearchParams(window.location.search); } catch (e) { return; }
    if (params.get("paid") !== "true") return;
    const saved = lsGet("pf_plan");
    let ok = false;
    if (saved) {
      try {
        ...
        if (d && d.plan) {
          if (d.f) setF(d.f);
          setPlan(d.plan);
          if (d.paceUnit) setPaceUnit(d.paceUnit);
          ok = true;
        }
      } catch (e) {}
    }
    setUnlocked(true);
    setStep(ok ? "results" : "landing");
...
Include 3-4 phases, exactly 7 days in weeklyStructure, 5 paces (Easy, Long Run, Tempo, Interval, Goal Race Pace), 2 strength sessions with 4-5 exercises each, and 5 tips.`;
    ...
    try {
      ...
      const pu = displayUnit === "imperial" ? "mi" : "km";
      setPaceUnit(pu);
      setPlan(result);
      // save so we can restore after a Stripe redirect
      lsSet("pf_plan", JSON.stringify({ f, plan: result, paceUnit: pu }));
      setStep(unlocked ? "results" : "paywall");
    } catch (e) {
  ...
  function payAndUnlock() {
    if (PAYMENT_LINK) {
      // plan is already saved; Stripe returns to ?paid=true to unlock
      try { window.location.href = PAYMENT_LINK; } catch (e) {}
    } else {
      // no payment configured yet — unlock so you can preview the full plan
      setUnlocked(true);
      setStep("results");
    }
  }
...
Show realistic progressive overload and a taper in the final 1-2 weeks. Exactly 3 highlights per week.`;
  ...
  function restart() {
    ...
    setErr(""); setOpenWeek(null); setUnlocked(false); lsDel("pf_plan");
  }
      ...
      <div className="pf-wrap">
        <div className="pf-top">
          ...
          <div className="pf-step-tag">
            {step === "landing" && "Coached by Arem"}
            {step === "form" && `Step ${wizard + 1} / 3`}
            {step === "loading" && "Working…"}
            {step === "error" && "Try Again"}
            {step === "paywall" && "Unlock Plan"}
...
/* ---------- paywall ---------- */
function Paywall({ plan, inputs, distance, price, priceWas, live, onPay, onBack }) {
  const a = plan.assessment || {};
  const feas = (a.feasibility || "ambitious").toLowerCase();
  return (
    <div className="pf-fade">
      <h2 className="pf-display" style={{ fontSize: "clamp(30px,6vw,48px)", marginBottom: "6px" }}>
        Your plan is <span style={{ color: "var(--volt)" }}>ready.</span>
      </h2>
      <p className="pf-step-desc" style={{ marginBottom: "22px" }}>Here's your goal verdict — unlock the full programme below.</p>

      {/* free teaser: the verdict + target */}
      ...
      <div className="pf-lock">
        ...
        <h3 className="pf-display" style={{ fontSize: "26px", letterSpacing: "0.03em" }}>Unlock your full {distance} plan</h3>
        <ul className="pf-lock-list">
          <li>Personalised training paces (km or miles)</li>
          <li>Full weekly structure — runs, tempo, intervals & long runs</li>
          <li>2 structured gym sessions with exercises &amp; sets</li>
          <li>Complete week-by-week schedule with taper</li>
          <li>Download as PDF &amp; email it to yourself</li>
        </ul>
        {priceWas && <div className="pf-price-badge">Launch offer · 50% off</div>}
        <div className="pf-price">
          {priceWas && <span className="pf-price-was">{priceWas}</span>}
          {price}<span> one-off</span>
        </div>
        <button className="pf-cta pf-lock-cta" onClick={onPay}>
          {live ? "Pay & Unlock →" : "Unlock Plan →"}
        </button>
        {!live && <p className="pf-lock-note">Payment isn't connected yet, so this unlocks for free in preview. Add your Stripe link to charge for real.</p>}
        <button className="pf-lock-back" onClick={onBack}>← Edit my details</button>
      </div>
    </div>
  );
}
